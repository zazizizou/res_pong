...
begin
	...
	enable_proc: process(l_paddle_hit_pulse, r_paddle_hit_pulse, l_scored_pulse, r_scored_pulse)
	begin
		...
		if(res_n = '0') then
			...
			if enb_music_l_paddle_hit = '1' then
				if counter <= "10111110101111000010000000" then -- counts to 0.5 sec
					sound_effect <= music_l_paddle_hit_A; 
					counter <= counter + 1;
				elsif counter >= "10111110101111000010000000" then
					sound_effect <= music_l_paddle_hit_C;
